...
def apply_model_to_blind_data(input_file, impute_model, model_filename, output_tag):
    ...
    with open(impute_model, 'rb') as fimpute:
        imputer = pickle.load(fimpute)
    ...
    # Print metrics
    print("Evaluation on input data:")
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python apply_model.py <input_file> <model_filename> <model_filename> <output_tag>")
        sys.exit(1)

    input_file = sys.argv[1]
    impute_model = sys.argv[2]
    model_filename = sys.argv[3]
    output_tag = sys.argv[4]
    apply_model_to_blind_data(input_file, impute_model, model_filename, output_tag)
